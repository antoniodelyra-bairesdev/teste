...
def make_response(
    ...
    if include_metadata and request:
        ...
        response_metadata.update({
            ...
            "user_agent": request.headers.get("user-agent", "unknown")[:100],  # Truncate user agent
        })

        # Include validation results if available
        if hasattr(request.state, 'validation_results'):
            validation_results = request.state.validation_results
            response_metadata["validation_status"] = "passed" if validation_results["is_valid"] else "failed"
            if validation_results["warnings"]:
                response_metadata["validation_warnings"] = len(validation_results["warnings"])
    ...
    if settings.DEBUG or getattr(settings, 'RESPONSE_LOGGING_ENABLED', False):
        log_info(f"Response created: {response_metadata['response_id']} | Status: {status_code}")
