...
class Analysis(BaseModel):
    ...
    created_at = Column("wiki_dt_created_at", DateTime, default=datetime.now, nullable=False)

    agent_id = Column("agen_cd_id", Integer, ForeignKey("agent.agen_cd_id"), nullable=False)
    agent = relationship("Agent", back_populates="analyses", uselist=False)

    wikiclip_id = Column("wiki_cd_id", Integer, ForeignKey("wikiclip.wiki_cd_id"),  nullable=False)
