...
class TestDBManager:
    ...
    async def test_transaction_success(self):
        ...
        async with db_manager.transaction() as session:
            ...
            session.add(TestDBModel(name="test"))
        ...
        try:
            async with db_manager.transaction() as outer_session:
                ...
                try:
                    ...
                    # Inner exception should propagate to outer transaction
                    pass
        except Exception:
            pass
...
def test_set_db_manager_in_request_config():
    ...
    with patch("ehp.db.db_manager.get_current_request", return_value=mock_request):
    ...
    with patch("ehp.db.db_manager.get_current_request", side_effect=Exception("Test error")), \
            patch("builtins.print") as mock_print:
        set_db_manager_in_request_config(db_manager)

        # Verify error is printed
        mock_print.assert_called_once()
        assert "Error setting db manager in request config" in mock_print.call_args[0][0]
